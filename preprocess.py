...
def preprocess(args, input_folders, out_dir, hparams):
	...
	write_metadata(args,metadata, out_dir)

def write_metadata(args,metadata, out_dir):
	with open(os.path.join(out_dir, 'train.txt'), args.file_mode, encoding='utf-8') as f:
		for m in metadata:
...
def main():
	...
	parser.add_argument('--file_mode', default='w')
