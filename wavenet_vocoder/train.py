				...
				try:
					...
					if (checkpoint_state and checkpoint_state.model_checkpoint_path):
						...
						if(args.transferlearn_dir):
							transsave_dir = os.path.join(args.transferlearn_dir, 'wave_pretrained')
							
							checkpoint_state = tf.train.get_checkpoint_state(transsave_dir)
							
							if (checkpoint_state and checkpoint_state.model_checkpoint_path):
								log('Loading transfer checkpoint {}'.format(checkpoint_state.model_checkpoint_path), slack=True)
								load_averaged_model(sess, sh_saver, checkpoint_state.model_checkpoint_path)

							else:
								log('No model to load at {}'.format(transsave_dir), slack=True)
